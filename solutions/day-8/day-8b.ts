...
function getAntinodes(inputs: string[]): {x: number, y: number}[] {
    ...
    signalsToSearch.forEach(signal => {
        ...
        lines.forEach(line => {
            ...
            let newPoint1 = {x: point1.x - dx, y: point1.y - dy};
            let newPoint2 = {x: point2.x + dx, y: point2.y + dy};

            while (newPoint1.x >= 0 && newPoint1.y >= 0 && newPoint1.y < inputs.length && newPoint1.x < inputs[newPoint1.y].length) {
                antinodes.push(newPoint1);
                newPoint1 = {x: newPoint1.x - dx, y: newPoint1.y - dy};
            }

            while (newPoint2.x >= 0 && newPoint2.y >= 0 && newPoint2.y < inputs.length && newPoint2.x < inputs[newPoint2.y].length) {
                antinodes.push(newPoint2);
                newPoint2 = {x: newPoint2.x + dx, y: newPoint2.y + dy};
            }
        });
    });
